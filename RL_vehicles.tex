...
dynamics of the vehicle, a point-mass kinematic model is used. \martin{For
updating the speed and position for time step $t + 1$ the Euler and
ballistic methods are used, respectively.}
\martin{The roman subscripts ('sub'') are only used for textual
  subscripts ($x\sub{max}$, $x\sub{phys}$), not for variable
  subscripts. There, the normal underscore is used}
\begin{equation}
	v_{t+1} = v_{t} + a_{t} d
\end{equation}
...
Both neural networks, critic and actor,\martinc{critic and actor are
  generic concepts, thus lowercase. Named policies (Free-Driving
  Policy or free-driving policy) may be written upper- or lowercase but
  always without a hyphen connecting the last word} are feed-forward neural
networks with one layer of hidden neurons, containing 16 neurons (see
Figure \ref{fig:MRL}). ReLU activation functions are used
\martinc{define ReLU or give a reference}.  The learning rate for
...
\martin{zero-reverting} Ornstein-Uhlenbeck process with $\Theta = 0.15$  and $\sigma = 0.2$ as suggested in \cite{DDPG}. The update rate of the target networks is set to 0.001.
...
its own speed $v$, the \martin{leader's speed $v_l$},
and the (bumper-to-bumper)
gap $g$ \martinc{replaced gap $s$ by gap $g$, also 
$s_0\to g_0, s^*\to g^*$.}
 Again, linear translation and scaling is used
to reduce the dimensions and to bring all observations approximately
into the same range of $[0,1]$. \martinc{The third state usually is in
  $[-1,1]$ and $g/g\sub{max}$ needs a fallback if there is no leader} The observation at time step $t$ is defined as
...
\end{equation}
%
where $g\sub{max}$ is set to  \unit[200]{m}.
...
comfortable deceleration $b\sub{comf}$, \martinc{a theta function
  $\Theta(v-v_l)=\Theta(-v_l-v)$ is missing in Eq.~\eqref{bkin}}
\martinc{Why a tanh function? \eqref{r2} has a kink at zero and
  flattens if the situation becomes really critical. Why not 
$-((b\sub{kin}-b\sub{comf})/a\sub{min})^2
  \Theta(b\sub{kin}-b\sub{comf})$ possibly normalized to a max of 1 if
  this is needed?}
...
r_1 = 
\begin{cases}
-\tanh\left(\frac{b_{kin}-b\sub{comf}}{\martin{-a\sub{min}}}\right),& \text{if } b_{kin}>b\sub{comf}\\
0,              & \text{otherwise}
\end{cases}
\end{equation}
...
\begin{equation}
\label{bkin}
b_{kin} = \frac{(v_l-v)^2}{g}
\end{equation}
The argument of the tanh function with  the
maximum possible deceleration (\martin{$\approx \unit[9]{m/s^2}$} on dry roads) gives a
...
distance to the leading vehicle. \martinc{cf. my comment in the chat of
  the meeting May 26. There, I suggested Erlang functions $r(x)=x^n
  \exp(-\lambda x)$, e.g., $x \exp(-\lambda x)$. 
with $1/\lambda=s-g\sub{opt}$. The fact that it is constant instead of linearly
  decreasing for large gaps does not matter since then the
  free-traffic regime takes over, anyway}
...
\martin{A non-emergency relative-speed term is missing, e.g.,
$r_4=-v\sub{des}/b\sub{comf} (v-v_l)\theta(v-v_l)/g$}
...
typical physical leader accelerations. \martin{Relating the acceleration to
...
\begin{equation}
\phi = \martin{\exp\left(-\frac{2a\sub{phys}d}{v_{l,\text{des}}}\right),}
\end{equation}
...
\ref{tab:AR1Parameters}. If the speed exceeds the defined range of
$[0, v_{l,\text{des}}]$, it is manually set to the range
limits. \martinc{Consider applying the lower limit, only, as discussed
  May 26}
\begin{figure}
	\centering
...
\begin{figure}
	...
	\caption{Response to an external leading vehicle speed
          profile. \martinc{Change velocity to speed in all plots}}
...
accelerations and decelerations are higher \martin{than that of the more
...
changed. \martin{Still, when simulating platoons of the three RL
  agents in any order, the accelerations and jerks decrease along the
  platoon.} \martinc{Probeweise simulieren, obwohl ich sicher bin,
  dass das der Fall ist. Durchaus auch mal extreme F\"alle wie
  $T=\unit[0.5]{s}$ oder gar \unit[0.2]{s} (mit $d=\unit[0.05]{s}$)
...
by \martinc{here, I have left $s^*$ since $g^*$ has already been used for the
gap reward function}
\begin{equation}
\label{eq:IDM}
...
\martinc{Du solltest auch mal testen, was passiert, wenn du die
  Reward-Funktionen zum Lernen anders parameterisiert. So ist es ja ein wenig
  unfair, da der RL-Agend beim Lernen schon die volle IDM-Information
  hat und durch die Dutzende (Hunderte?) von internen
  Synapsenparameter der verschiedenen Layers durch mehr Parameter
  eigentlich nur besser werden kann. Ich gehe davon aus, dass dann die
  Fitfunktion irgendwann schlechter wird, aber die Reward-Funktion
  nicht. Just exploring; vielleicht ergeben sich interessante
  Erkenntnisse ...}
\begin{table}
...
\martin{real}  trajectory data,\martinc{Die Napoli-Daten sind
  *nicht* \emph{naturalistic}. So nennt man nur Daten von Fahrern, die
  nicht wissen, dass sie beobachtet werden; die Napoli-Daten sind im
  Gegensatz dazu 
  \emph{experimental} data, aber natuerlich \emph{real}} bringing the model to its limits. 
