...
\begin{document}
	...
	\begin{equation}
	\label{rt1}
	r_{t, \rm free-driving} =r_{t, \rm speed} + w\sub{jerk} r_{t, \rm jerk} ,
	\end{equation}
	%
	...
	To match the action space of the Free-Driving Policy, the acceleration is analogously defined as a continuous variable in the range between $a\sub{\rm min} = \unit[-9]{m/s^2}$ and $a\sub{\rm max} =\unit[2]{m/s^2}$.	
	...
	\begin{equation}
	\label{rt2}
	r_{t, \rm car-following} = r_{t, \rm safe} + w\sub{gap}r_{t,\rm gap}+w\sub{jerk}r_{t, \rm jerk} ,
	\end{equation}
