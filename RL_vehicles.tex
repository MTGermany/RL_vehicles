...
gap $s$ to the leader. Again, linear translation and scaling is used
...
into the same range of $[0,1]$. \martinc{The third state usually is in
  $[-1,1]$ and $s/s\sub{max}$ needs a fallback if there is no leader} The observation at time step $t$ is defined as
